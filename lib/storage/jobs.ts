...
const STORAGE_KEY = "local:jobs" as const;
...
export async function createJob(
	...
	const jobs = (await storage.getItem<ScrapeJob[]>(STORAGE_KEY)) || [];
	jobs.push(job);
	await storage.setItem(STORAGE_KEY, jobs);

	return job;
}

export async function listJobs(): Promise<ScrapeJob[]> {
	const data = (await storage.getItem<ScrapeJob[]>(STORAGE_KEY)) || [];
...
export async function cleanupOldJobs(): Promise<void> {
	...
	await storage.setItem(STORAGE_KEY, kept);
}
